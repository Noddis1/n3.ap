...
client.on("interactionCreate", async (interaction) => {
  if (interaction.isStringSelectMenu()) {
    // Handle select menu interactions (e.g., build_select dropdown)
    const customId = interaction.customId;
    if (customId === "build_select") {
      const selected = interaction.values[0]; // e.g., "build_0"
      const buildIndex = parseInt(selected.split("_")[1]);
      const builds = [
        { name: "Starter Kit", size: "50M", queue: 12, time: "2-3 timer", price: "kontakt for pris" },
        { name: "Standard Pack", size: "150M", queue: 8, time: "4-6 timer", price: "kontakt for pris" },
        { name: "Premium Build", size: "350M", queue: 5, time: "8-12 timer", price: "kontakt for pris" },
        { name: "Custom XL", size: "500M+", queue: 3, time: "24+ timer", price: "kontakt for pris" },
      ];
      ...
      if (build) {
        const embed = new djs.EmbedBuilder()
          .setTitle(`📦 ${build.name}`)
          .addFields(
            { name: "Størrelse", value: build.size, inline: true },
            { name: "Estimert tid", value: build.time, inline: true },
            { name: "Gjeldende kø", value: `${build.queue} personer`, inline: true },
            { name: "Pris", value: build.price, inline: false }
          )
          .setColor(0xFF8C00)
          .setFooter({ text: "React eller DM for å starte orden" });
