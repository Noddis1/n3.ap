...
const API_URL = "https://api.base44.com/api/apps/69e0f31e9fcd866ea1f3ea35/entities/BotCommand";
...
client.on("interactionCreate", async (interaction) => {
  ...
  const cmd = panelCommands.find(c =>
    c.name.toLowerCase().replace(/\s+/g, "-") === interaction.commandName
  );
  if (!cmd) return;
...
client.on("messageCreate", async (message) => {
  ...
  const cmd = panelCommands.find(c =>
    ...
    c.name.toLowerCase().replace(/\s+/g, "-") === cmdName
  );
  if (!cmd) return;
